...
properties {
  ...
  $namespace = "Repo"
  $vs2005Directory = "$rootSourceDirectory\$namespace.Integration"
}
...
task BuildETL {
	...
	write-host "$vs2005Directory\$name\$compressionName" 
	write-host "$rootBuildDirectory\$name\$compressionName"
	
	CopyFiles "$vs2005Directory\$name" "$rootBuildDirectory\$name" *.dtsx,*.dtsconfig,*.cmd
	CopyFiles "$vs2005Directory\$name\$compressionName" "$rootBuildDirectory\$name\$compressionName" *
	CopyFiles "$vs2005Directory\$name\$dataFileDownload" "$rootBuildDirectory\$name\$dataFileDownload" *
	#CopyFiles "$vs2005Directory\$name\$generateReports" "$rootBuildDirectory\$name\$generateReports" *
